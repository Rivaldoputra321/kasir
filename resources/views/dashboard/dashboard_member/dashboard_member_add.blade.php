...
<div class="card">
  <div class="card-body">
    ...
    <form action="{{route('dashboard_member.store')}}" method="POST">
      @csrf
      <div class="row">
        <div class="col-12 mb-3 mt-4">
          ...
          <input type="text" name="alamat" class="form-control" placeholder="Alamat" required value="" autofocus style="height: 5rem;">
          <label for="password" class="form-label">No-Telpon</label>
          <input type="text" name="no_telp" class="form-control  @error('no_telp') is-invalid @enderror" placeholder="No Telpon" required value="{{old('no_telp')}}" autofocus>
          @error('no_telp')
            <div class="invalid-feedback">
              {{ $message }}
            </div>
